                ...
                column(width = 12,
                       box(title = "Hasil Eksplorasi", status = "success", solidHeader = TRUE, width = "100%",
                           tabBox(
                             ...
                             tabPanel("Peta Sebaran (Variabel X)", icon = icon("map-location-dot"),
                                      leafletOutput("explore_map", height = "600px"),
                                      hr(),
                                      uiOutput("explore_map_interpretasi")),
...
server <- function(input, output, session) {
  ...
  explore_map_obj <- reactive({
    req(input$explore_var_x)
    df <- explore_data_filtered()
    var_x <- tolower(input$explore_var_x)
    
    pal <- colorNumeric(palette = "YlOrRd", domain = df[[var_x]])
    
    popup <- paste0("<strong>", df$nmkab, "</strong><br/>",
                    str_to_title(var_x), ": ", round(df[[var_x]], 2))
    
    leaflet(df) %>%
      addProviderTiles(providers$CartoDB.Positron) %>%
      addPolygons(fillColor = ~pal(df[[var_x]]), weight = 1, color = "white", fillOpacity = 0.8, popup = popup) %>%
      addLegend(pal = pal, values = ~df[[var_x]], title = str_to_title(var_x), position = "bottomright")
  })
  
  output$explore_map <- renderLeaflet({ explore_map_obj() })
  ...
  observeEvent(input$save_eksplorasi, {
    ...
    add_to_report(list(
      title = paste("Eksplorasi - Peta Sebaran", input$explore_var_x),
      type = "leaflet",
      content = explore_map_obj()
    ))
    add_to_report(list(
  ...
  output$hasil_analisis_tabs <- renderUI({
    ...
    tabBox(
      ...
      tabPanel("Visualisasi Peta", icon = icon("map-location-dot"), leafletOutput("peta_kategori", height = "600px")),
  ...
  manajemen_map_obj <- reactive({
    req(hasil_kategorisasi(), !hasil_kategorisasi()$error)
    df <- hasil_kategorisasi()$data_lengkap
    
    pal_kat <- colorFactor(palette = "viridis", domain = df$kategori, na.color = "transparent")
    
    popup_content <- paste0("<strong>", df$nmkab, "</strong><br/>",
                            "Kategori: ", df$kategori, "<br/>",
                            "Nilai Asli: ", round(df[[tolower(input$var_kategorisasi)]], 2))
    
    leaflet(df) %>%
      addProviderTiles(providers$CartoDB.Positron) %>%
      addPolygons(fillColor = ~pal_kat(kategori), weight = 1, color = "white", fillOpacity = 0.8,
                  popup = popup_content) %>%
      addLegend(pal = pal_kat, values = ~kategori, title = "Kategori", position = "bottomright", na.label = "NA")
  })
  output$peta_kategori <- renderLeaflet({ manajemen_map_obj() })
  ...
  observeEvent(input$save_manajemen, {
    ...
    add_to_report(list(
      title = paste("Manajemen Data - Peta Kategori untuk", input$var_kategorisasi),
      type = "leaflet",
      content = manajemen_map_obj()
    ))
    add_to_report(list(
  ...
  # --- SERVER LOGIC UNTUK ANALISIS KLASTER ---
  ...
  cluster_results <- eventReactive(input$cluster_btn, {
    ...
    map_with_clusters <- map_data_full_raw %>%
      right_join(data_with_clusters, by = "kodeprkab")
  ...
  output$cluster_results_ui <- renderUI({
    ...
    box(title = "Hasil Analisis Klaster", status = "success", solidHeader = TRUE, width = NULL,
        tabBox(
          width = 12,
          tabPanel("Peta Sebaran Klaster", icon = icon("map-marked-alt"), leafletOutput("cluster_map", height = "600px")),
  ...
  cluster_map_obj <- reactive({
    req(cluster_results())
    res <- cluster_results()
    
    pal_cluster <- colorFactor(palette = "viridis", domain = res$map_data$Klaster)
    
    popup_content <- paste0("<strong>", res$map_data$nmkab.x, "</strong><br/>",
                            "Provinsi: ", res$map_data$nmprov.x, "<br/>",
                            "Klaster: ", res$map_data$Klaster)
    
    leaflet(res$map_data) %>%
      addProviderTiles(providers$CartoDB.Positron) %>%
      addPolygons(fillColor = ~pal_cluster(Klaster),
                  weight = 1, color = "white", fillOpacity = 0.8,
                  popup = popup_content) %>%
      addLegend(pal = pal_cluster, values = ~Klaster, title = "Klaster", position = "bottomright")
  })
  output$cluster_map <- renderLeaflet({ cluster_map_obj() })
  ...
  observeEvent(input$save_klaster, {
    req(cluster_results())
    add_to_report(list(
      title = paste("Analisis Klaster - Peta Sebaran", input$cluster_k, "Klaster"),
      type = "leaflet",
      content = cluster_map_obj()
    ))
    add_to_report(list(
